...
// Checkin $Date: June 29, 2010   09:55:23 $
//###########################################################################
//
// FILE:   Example_2803xLina_EXALB.c
//
// TITLE:  DSP2803x Device LIN-A External Analog Loop Back program.
//
//
// ASSUMPTIONS:
//
...
//
//	  Connect the LIN module TD and RD pins to a LIN transciever
...
//	  LIN bus.  The LIN module recieves this data off the LIN bus.
//	  Correct data transmission is verified. The LIN pins should be connected
//	  to a LIN transceiver.
//
//    Watch Variables:
//       Rx_Data_LIN_MASTER
//       Tx_Data_LIN_MASTER
...
interrupt void LINa_Lvl0_Isr(void);
interrupt void LINa_Lvl1_Isr(void);
...
void main()
{
	...
	//Initialize sent and recieved data pointers
    ...
    *****************************************************************/
    ...
    for(i=0;i<8;i++)
    {
		...
		//be placed on the bus and also read into the recieve buffers
		...
		//Transfer data from recieve data buffers to data arrays
	...
	//Validate correct interrupt occurence
	if(intCount != 16)
	{
		error |= 2;
	}
