...
PWM_P = 12
PWM_N = 0
PWM_PERIOD = 4.0/(6.0*pow(10,7))

def getActiveCurrent(step, varepsilon, fillCoeff):
    T = PHASE_L_H / PHASE_R_OHM
    sigma = T/PWM_PERIOD
    sigma = 1/sigma
    test = 1 - math.exp( -sigma + sigma * fillCoeff )
    i_n = ( PWM_P / PHASE_R_OHM )                                               \
            * ( 1                                                               \
                - ( math.exp( -sigma * varepsilon )/( 1 - math.exp(-sigma)) )   \
...
print ( getActiveCurrent( 0, 0.3, 0.5) )
print ( getPassiveCurrent(0, 0.6, 0.5) )
